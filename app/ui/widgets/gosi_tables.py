...
class PriceEditDelegate(QStyledItemDelegate):
    ...
    def createEditor(self, parent, option, index):
        ...
        editor.setStyleSheet(
            "background-color: #FFFFFF; color: #000000; padding: 0 4px;"
        )
        return editor
    ...
    def setModelData(self, editor, model, index):
        ...
        if value is None:
            return
        model.setData(
            index, min(max(value, self._min), self._max), Qt.ItemDataRole.EditRole
        )


class GosiBaseTable(QTableWidget):
    rowSelected = Signal(dict)  # 행 클릭 시 출처(ref) 표시 시스널
        ...
        for item in rows:
            ...
            self._add_row(
                [
                    ...
                    (
                        f"{item['조견표']:,}원"
                        if isinstance(item["조견표"], (int, float))
                        else "—"
                    ),
                    (
                        f"{item['고시']:,}원"
                        if isinstance(item["고시"], (int, float))
                        else "—"
                    ),
                    item["결과"],
                ],
                item["출처"],
                None if item["결과"] == "일치" else color,
            )
        self._fit_height()
    ...
    def fill_data(self, prices_data: dict):
        ...
        for service, items in prices_data.items():
            for key, item in items.items():
                bonus = item.get("가산수당")
                self._add_row(
                    [service, key, f"{item['금액']:,}", f"{bonus:,}" if bonus else ""],
                    item["출처"],
                )
        self._fit_height()
