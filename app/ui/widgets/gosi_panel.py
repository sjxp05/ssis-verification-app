...
class GosiDocumentViewer(QTextBrowser):
    ...
    def render_document(self, highlight: dict | None = None) -> None:
        if not self._blocks:
            self.setHtml(
                f"<style>{_DOC_CSS}</style><p class='body'>고시 파일을 불러오면 원문이 표시됩니다.</p>"
            )
            return
        ...
        for block in self._visible_blocks():
            if block["종류"] == "문단":
                parts.append(
                    f"<p class='{_para_class(block['글'])}'>{_escape(block['글'])}</p>"
                )
                continue
            ...
            for r, row in enumerate(block.get("격자", [])):
                ...
                for c, cell_info in enumerate(row):
                    ...
                    anchor_tag = (
                        f"<a name='{_anchor(block['표번호'], r)}'></a>"
                        if hit and c == 0
                        else ""
                    )
                    parts.append(
                        f"<td class='{css_class}'>{anchor_tag}{cell_html}</td>"
                    )
                parts.append("</tr>")
            parts.append("</table>")
