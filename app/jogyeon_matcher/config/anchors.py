...
#
# 값 추출기(services/jogyeon_value_extractor.py)와 라벨 매처가 함께 쓴다.
# 양쪽에 따로 두면 한쪽만 고쳤을 때 매처는 옛 라벨을 감시하고 추출기는 새 라벨을
# 읽는 상태가 되므로, 정의는 여기 한 곳에만 둔다.
...
# 표 스코프 안에서 유일해야 하는 앵커. 여러 개면 어느 셀을 읽을지 정할 수 없다.
SCALAR_ANCHORS = (
    BASE_PRICE, A_VALUE, BASIC_RATE, CAP_LABEL,
    GRADE_HEADER, INCOME_HEADER, JH_BASIC, JH_EXTENDED,
)
