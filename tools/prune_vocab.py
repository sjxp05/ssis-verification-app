# 어휘 축소 — 임베딩 테이블에서 쓰지 않는 문자 체계를 덜어낸다 (빌드 타임 전용)
#
# export_model.py 가 불러 쓴다. 직접 실행할 일은 없다.
#
# 원본 e5-small 의 어휘 250,002개 중 한글은 2.2%뿐인데 임베딩 테이블이 모델의
# 82%를 차지한다. 쓰지 않을 체계를 지우면 용량이 크게 준다.
#
# 채택 수위는 T3(한글 + ASCII 라틴 + 중립). 실측 결과 원본과 임베딩 코사인
# 일치도가 1.0000 으로 손실이 0 이면서 용량은 52% 줄었다. 우리 라벨이 한글·숫자·
# 일부 ASCII 로만 이뤄져 한자·가나·악센트 라틴 토큰은 애초에 선택되지 않는다.
#
# 한 단계 더(T4, 다문자 라틴 제거)는 일치도가 0.848 로 떨어진다.
