...
def gen_jogyeon(path: Path, year: int, variant: bool, bloat: bool):
    ...
    ws.title = "인정조사"
    ws["B1"] = "기본단가"
    ws["C1"] = int(17270 * bump)
    ws["B2"] = "A값"
    ws["C2"] = int(3089062 * bump)
    ws["D2"] = int(154400 * bump)
    ws["B3"] = "기본 부담률"
    ...
    for block in range(BULK_BLOCKS):
        ...
        for i in range(BULK_ROWS):
            ...
            if variant:
                if n % 12 == 5:
                    label = (
                        f"{prefix} {n + 1}차 적용 대상(개정)"  # 개칭 → 하이브리드 유도
                    )
                elif n % 12 == 9:
...
def gen_hwpx(path: Path):
    ...
    def tbl(rows):
        ...
        for row in rows:
            ...
            for cell in row:
                out.append(
                    f"<hp:tc><hp:subList><hp:p><hp:run><hp:t>{escape(str(cell))}</hp:t></hp:run></hp:p></hp:subList></hp:tc>"
                )
            out.append("</hp:tr>")
        out.append("</hp:tbl>")
        return "".join(out)
    ...
    parts.append(
        tbl(
            [
                ...
                [
                    "「의료법」에 따른 의료기관",
                    "대상자가 의료기관을 방문하여 발급, 의사가 가정을 방문하여 발급",
                    f"{won(20790)} {won(72740)}",
                ],
                [
                    "「지역보건법」에 따른 보건소",
                    "대상자가 보건소를 방문하여 발급, 의사가 가정을 방문하여 발급",
                    f"{won(15790)} {won(57740)}",
                ],
            ]
        )
    )
    ...
    parts.append(
        tbl(
            [["활동지원급여 구간", "종합점수", "금액"]]
            + [
                [f"{i + 1}구간", f"{465 - 30 * i}점 이상", won(8472000 - 405000 * i)]
                for i in range(15)
            ]
        )
    )
    parts.append(
        tbl(
            ...
            + [
                [f"{i + 1}구간", won(5940000 - 405000 * i), won(5840000 - 405000 * i)]
                for i in range(15)
            ]
        )
    )
...
def bench_match(baseline, target, runs=2):
    ...
    wrap(
        ...
        "table_analyzer.check_anchor_uniqueness",
    )
...
def bench_extract(sheet_path, runs=2):
    ...
    for i in range(runs):
        _reset()
        t0 = time.perf_counter()
        data = jve.ValueExtractor().extract_jogyeon_values(
            UploadedFile.from_path(sheet_path)
        )
        total = time.perf_counter() - t0
        out.append(
            report(
                "2. 조견표 값 추출",
                total,
                f"(run{i + 1}) 키 {sum(len(d) for d in data)}개",
            )
        )
    return out
    ...
    for i in range(runs):
        ...
        out.append(
            report(
                "4. 결제단가표(작년) 로딩",
                total,
                f"(run{i + 1}) {df.shape[0]}행 x {df.shape[1]}열",
            )
        )
    return out
...
def main():
    ...
    if args.big:
        ...
        suffix += "_big"
    baseline = (
        Path(args.baseline) if args.baseline else DATA / f"jogyeon_2026{suffix}.xlsx"
    )
